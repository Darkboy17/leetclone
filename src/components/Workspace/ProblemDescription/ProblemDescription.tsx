...
const ProblemDescription: React.FC<ProblemDescriptionProps> = ({
  ...
  const handleLike = async () => {
    ...
    // if already liked, disliked or neither
    // will be using firebase transactions
    if (updating) return;
  ...
  const handledislike = async () => {
    ...
    await runTransaction(firestore, async (transaction) => {
      const { userDoc, problemDoc, userRef, problemRef } =
        await returnUserDataAndProblemData(transaction);
      if (userDoc.exists() && problemDoc.exists()) {
        //cases : already disliked, already liked, or neither
        if (disliked) {
          transaction.update(userRef, {
            dislikedProblems: userDoc
              .data()
  ...
  const handleStar = async () => {
    if (!user) {
      toast.error("You must be logged in to give the problem a star", {
        position: "top-left",
        theme: "dark",
      });
      return;
...
// A hook to fetch data based on problem selected
